...
class ExactInference:
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float):
        ...
        for i in range(self.belief.getNumRows()):
            for j in range(self.belief.getNumCols()):
                x = util.colToX(j)
                y = util.rowToY(i)
                distance = pow(pow(x-agentX, 2) + pow(y-agentY, 2), 0.5)
                prop = util.pdf(distance, Const.SONAR_STD, observedDist)
                self.belief.setProb(i, j, self.belief.getProb(i,j)*prop)
    ...
    def elapseTime(self) -> None:
        ...
        new_blief = util.Belief(self.belief.getNumRows(), self.belief.getNumCols(), 0)
        for ((old, new), trans) in self.transProb.items():
            old_prop = self.belief.getProb(old[0], old[1])
            new_blief.addProb(new[0], new[1], old_prop*trans)
        new_blief.normalize()
        self.belief = new_blief
        # END_YOUR_CODE
...
class LikelihoodWeighting:
    ...
    def updateBelief(self) -> None:
        ...
        for i, (r, c) in enumerate(self.samples):
            newBelief.addProb(r, c, self.weights[i])
        newBelief.normalize()
    ...
    def elapseTime(self) -> None:
        ...
        newSamples = []
        for i, (r, c) in enumerate(self.samples):
            if (r, c) not in self.transProbDict:
                newSamples.append((r, c)) 
            else:
                newSamples.append(util.weightedRandomChoice(self.transProbDict[(r, c)]))
...
class ParticleFilter:
    ...
    def __init__(self, numRows: int, numCols: int):
        ...
        all_pos = [(x,y) for x in range(numRows) for y in range(numCols) if (x, y) in self.transProbDict]
        for p in range(self.NUM_PARTICLES):
            pos = random.choice(all_pos)
            self.particles[pos] = self.particles.get(pos, 0) + 1
        # END_YOUR_CODE
        self.updateBelief()
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for pos in self.particles:
            x = util.colToX(pos[0])
            y = util.rowToY(pos[1])
            dis = math.sqrt((agentX - x) ** 2 + (agentY - y) ** 2)
            prop = util.pdf(dis, Const.SONAR_STD, observedDist)
            weight[pos] = self.particles[pos] * prop
